 ...
 *
 * @example
 * const mimeType = utils.getArchiveMimeType('path/to/raspberrypi.img.gz');
 */
exports.getArchiveMimeType = (filename) => {
  if (mime.lookup(filename)) {
    return Bluebird.resolve(mime.lookup(filename));
  }

  const FILE_TYPE_ID_BYTES = 261;

  return Bluebird.using(fs.openAsync(filename, 'r').disposer((fd) => {
    return fs.closeAsync(fd);
  }), (fd) => {
    const BUFFER_START = 0;
    const buffer = Buffer.alloc(FILE_TYPE_ID_BYTES);

    return fs.readAsync(fd, buffer, BUFFER_START, FILE_TYPE_ID_BYTES, null).then(() => {
